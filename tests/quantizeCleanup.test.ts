...
test('groupCompoundPaths punches holes only when fully contained', () => {  const square = (x0, y0, x1, y1) => ({
  ...
  const hasPoint = (d, x, y) => {
    const nums = d.match(/-?\d+(\.\d+)?/g) || [];
    for (let k = 0; k + 1 < nums.length; k += 2) {
      if (Math.abs(+nums[k] - x) < 0.5 && Math.abs(+nums[k + 1] - y) < 0.5) return true;
    }
    return false;
  };
  const compound = result.find((d) => subpathCount(d) === 2)!;
  const standalone = result.find((d) => subpathCount(d) === 1 && !hasPoint(d, 0, 0))!;
  assert.ok(compound, 'outer+hole compound exists');
  assert.ok(hasPoint(compound, 40, 40) || hasPoint(compound, 60, 60), 'hole subpath present in the compound');
  assert.ok(standalone, 'edge-hugging contour stays standalone');
  assert.ok(hasPoint(standalone, 95, 40), 'standalone is the edge-hugging contour');
});
...
test('groupCompoundPaths keeps islands inside holes as filled (donut center)', () => {
  ...
  const islandCompound = result.find((d) => subpathCount(d) === 1 && hasPoint(d, 80, 80));
  const bgCompound = result.find((d) => subpathCount(d) === 2 && hasPoint(d, 0, 0));

  assert.ok(islandCompound, 'island must render as its own filled compound');
  assert.ok(bgCompound, 'bg compound with the ring hole exists');
  assert.ok(hasPoint(bgCompound, 40, 40), 'ring hole must be punched from the background');
  assert.ok(!hasPoint(bgCompound, 80, 80), 'island must not be punched out of the background');
});
